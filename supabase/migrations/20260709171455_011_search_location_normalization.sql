...
-- accepts_gym may already exist from migration 002, so handle gracefully
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'trainers' AND column_name = 'accepts_gym'
  ) THEN
    ALTER TABLE public.trainers ADD COLUMN accepts_gym BOOLEAN NOT NULL DEFAULT FALSE;
  END IF;
END$$;
