...
import {
  View, Text, StyleSheet, ScrollView, TouchableOpacity, RefreshControl, Linking, Modal,
} from 'react-native';
...
import {
  ChevronLeft, Phone, X, ChevronDown, CheckCircle, Users, MessageSquare,
} from 'lucide-react-native';

type LeadStatus = Lead['status'];
type LeadRow = Lead & { student?: { full_name: string; phone: string | null } };
...
const TABS: { key: 'all' | 'pending' | 'contacted' | 'converted' | 'lost'; label: string }[] = [
  { key: 'all',       label: 'Todos' },
...
function cleanMessage(msg: string | null): string | null {
  if (!msg) return null;
  return msg.replace(/^(Nome|Telefone|Objetivo):\s*[^\n]+\n?/gim, '').trim() || null;
}
...
export default function LeadsScreen() {
  ...
  const [statusPickerLead, setStatusPickerLead] = useState<LeadRow | null>(null);

  const load = useCallback(async () => {
    if (!profile) return;
    const { data } = await supabase
      .from('leads')
      .select('*, student:profiles!leads_student_id_fkey(full_name, phone)')
  ...
  const getName = (lead: LeadRow) => lead.student?.full_name ?? extractName(lead.message) ?? 'Aluno';
  const getPhone = (lead: LeadRow) => lead.student?.phone ?? extractPhone(lead.message) ?? null;
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.header}>
        ...
        <View style={s.headerTitleWrap}>
          ...
          <Text style={s.headerSub}>Interessados no seu perfil</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
        ...
        {loading ? (
          ...
          <View style={s.emptyBox}>
            <View style={s.emptyIconWrap}>
              <Users size={28} color={Colors.neutral[300]} />
            </View>
            <Text style={s.emptyTitle}>Nenhum lead aqui</Text>
            <Text style={s.emptyDesc}>
              {tab === 'pending' ? 'Nenhuma nova solicitação pendente.' : 'Leads aparecem quando alunos te acionam.'}
            </Text>
          </View>
        ) : (
          <View style={s.list}>
            {filtered.map((lead) => {
              ...
              return (
                <TouchableOpacity key={lead.id} style={s.card} onPress={() => setDetailLead(lead)} activeOpacity={0.75}>
                  <View style={[s.cardBar, { borderLeftColor: cfg.color }]}>
                    ...
                    <View style={s.cardRight}>
                      ...
                      {phone && (
                        <TouchableOpacity style={s.waBtn} onPress={(e) => { e.stopPropagation?.(); openWhatsApp(phone); }}>
          ...
          <View style={[s.modalCard, { paddingBottom: Math.max(insets.bottom, Spacing.xl) }]}>
            ...
            {detailLead && (() => {
              ...
              const rows: [string, string, boolean?][] = [
                ...
                ['Status', STATUS_CONFIG[detailLead.status].label],
              ];
              return (
                <>
                  {rows.map(([label, val, isPhone], i) => (
                    ...
                    >
                      ...
                      <Text style={[s.detailValue, isPhone && { color: Colors.primary[600], textDecorationLine: 'underline' }]}>
                        {val}
                      </Text>
                    </TouchableOpacity>
                  ))}
                  ...
                  <Text style={[s.detailLabel, { marginTop: Spacing.md }]}>Alterar status</Text>
...
const s = StyleSheet.create({
  ...
  tabDot: { backgroundColor: Colors.warning[500], borderRadius: 99, paddingHorizontal: 5, minWidth: 16, alignItems: 'center' },
  ...
  card: {
    ...
    overflow: 'hidden', ...Shadows.sm, borderWidth: 1, borderColor: Colors.neutral[100],
  },
  cardBar: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    paddingHorizontal: Spacing.md, paddingVertical: 12, borderLeftWidth: 4,
  },
  ...
  statusBadge: {
    ...
    paddingHorizontal: 8, paddingVertical: 4, borderRadius: BorderRadii.full, borderWidth: 1,
  },
  statusBadgeText: { fontSize: 11, fontWeight: '700' },
  waBtn: { width: 30, height: 30, borderRadius: 15, backgroundColor: '#16A34A', alignItems: 'center', justifyContent: 'center' },
  msgPreview: { paddingHorizontal: Spacing.md, paddingBottom: 10, borderTopWidth: 1, borderTopColor: Colors.neutral[100], paddingTop: 8 },
  msgPreviewText: { fontSize: FontSizes.sm, color: Colors.neutral[500], lineHeight: 18 },

  modalBg: { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'flex-end' },
  modalCard: { backgroundColor: Colors.white, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: Spacing.xl, maxHeight: '90%' },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Spacing.md },
  modalTitle: { fontSize: FontSizes.xl, fontWeight: '700', color: Colors.neutral[900] },
  detailRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 11, borderBottomWidth: 1, borderBottomColor: Colors.neutral[100], gap: 12 },
  ...
  detailMsgBox: { marginTop: Spacing.sm, backgroundColor: Colors.neutral[50], borderRadius: BorderRadii.lg, padding: Spacing.md },
  ...
  statusOpt: { paddingHorizontal: 12, paddingVertical: 7, borderRadius: BorderRadii.full, borderWidth: 1.5, borderColor: Colors.neutral[200], backgroundColor: Colors.white },
  statusOptText: { fontSize: FontSizes.sm, fontWeight: '600', color: Colors.neutral[700] },
  waBtnFull: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#16A34A', borderRadius: BorderRadii.lg, paddingVertical: 12, marginTop: 4 },
  ...
  pickerCard: { backgroundColor: Colors.white, borderRadius: BorderRadii.xl, padding: Spacing.lg, width: 240, ...Shadows.lg },
  pickerTitle: { fontSize: FontSizes.md, fontWeight: '700', color: Colors.neutral[800], marginBottom: Spacing.sm },
  pickerOpt: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 10, paddingHorizontal: Spacing.sm, borderRadius: BorderRadii.md },
