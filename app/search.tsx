...
function parseLocationQuery(raw: string): QueryIntent {
  ...
  // "Cidade, Bairro" or "Cidade - Bairro"
  if (parts.length >= 2) {
    return { nameQuery: '', cityQuery: parts[0], neighborhoodQuery: parts.slice(1).join(' ') };
  }

  // Single token: could be city, neighborhood, or name
...
function applyLocationFilter(
  ...
  // If query has 2 parts → strict city+neighborhood match
  ...
  // Single token: try exact city match first
  const cityMatches = trainers.filter((t) =>
    onlineActive && t.accepts_online ? true : isExactCity(t)
  );

  if (cityMatches.length > 0) {
    return { filtered: cityMatches, locationLabel: intent.cityQuery };
  }
  ...
  if (nbhMatches.length > 0) {
    return { filtered: nbhMatches, locationLabel: intent.neighborhoodQuery };
  }

  // Fall back to name/specialty fuzzy match
...
export default function PublicSearch() {
  ...
  const compute = useCallback((trainers: TrainerWithProfile[], q: string, f: Filters) => {
    ...
    // relevance: keep default (featured + rating from DB)
        ...
        <View style={s.desktopBody}>
          ...
          <ScrollView style={s.desktopResults} showsVerticalScrollIndicator={false} contentContainerStyle={s.scrollContent}>
            ...
            <ResultsList
              ...
              featured={featured} regular={regular} query={query} locationLabel={locationLabel}
...
function ResultsList({
  loading, results, isFiltering, featured, regular, query, locationLabel, openWhatsApp,
}: {
  loading: boolean;
...
function CtaBanner() {
  return (
    <View style={s.ctaBanner}>
      ...
      <Text style={s.ctaBannerSub}>Crie sua conta grátis para entrar em contato e agendar sessões.</Text>
...
function TrainerSearchCard({
  ...
  const loc = [t.neighborhood, t.profile.city, t.profile.state]
    .filter(Boolean).join(', ');
...
const s = StyleSheet.create({
  ...
  topNav: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 12 },
  backBtn: {
  ...
  ctaBanner: {
    ...
    backgroundColor: Colors.primary[900], borderRadius: 20, padding: Spacing.lg, gap: 8,
  },
  ...
  // Filter panel (shared between sidebar and modal)
  filtersPanel: { padding: 20, gap: 0 },
